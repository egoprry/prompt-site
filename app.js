...
(() => {
  ...
  /* Thumbnail path for a resource image: dir/file -> dir/thumbs/file
     (generated by npm run build). */
  const thumbUrl = (post, file) => {
    const parts = file.split('/');
    const name = parts.pop();
    return imageUrl(post, [...parts, 'thumbs', name].join('/'));
  };
  ...
  function renderList(state) {
    ...
    if (shown.length === 0) {
      ...
      const buildCard = (p) => {
        ...
        const resFiles = [
          ...p.images.filter((f) => !isFin(f) && !isMult(f)),
          ...(p.refs ? Object.entries(p.refs).flatMap(([d, files]) => files.map((f) => `${d}/${f}`)) : []),
        ];
        const cardThumbs = resFiles.length ? `
  ...
  app.addEventListener('click', (e) => {
    ...
    if (cardThumb) {
      const group = [...cardThumb.closest('.card-thumbs').querySelectorAll('[data-card-thumb]')];
      lightboxImages = group.map((t) => t.getAttribute('data-full'));
      openLightbox(group.indexOf(cardThumb));
      return;
    }
